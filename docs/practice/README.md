...
GitHub Actions выполняет установку зависимостей, полный набор серверных тестов и production-сборку при каждом изменении ветки `main`. Qlty отдельно анализирует сопровождаемость и проблемы качества кода. Первый анализ ветки `main` завершился успешно с оценкой сопровождаемости **C**; текущие результаты GitHub Actions и Qlty видны в бейджах README.
